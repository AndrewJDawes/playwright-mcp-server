...
ENTRYPOINT ["npx", "-y", "@playwright/mcp@latest", "--browser", "chromium", "--port", "8931", "--isolated", "--allowed-hosts", "*", "--no-sandbox"]
CMD ["--headless"]
...
RUN echo '#!/bin/bash\n\
    ...
    su -c "cd /app && npx -y @playwright/mcp@latest --browser chromium --port 8931 --isolated --allowed-hosts * --no-sandbox $@" playwright\n\
